...
@testset "optimize mps Ising_Triangle_bad with $atype $updown updown" for updown = [false], atype = [CuArray]
    Random.seed!(100)
    β = 1
...
@testset "optimize mps Ising_Triangle_bad2 residual entropy with $atype $updown updown" for updown = [true], atype = [CuArray]
    Random.seed!(100)
    β = 100
    model = Ising_Triangle_bad2(1, 1, β)
    M = reshape(atype(model_tensor(model, Val(:Sbulk))),2,2,2,2)
    Au, Ad = optimisemps(M; χ = 128, mapsteps = 1, f_tol = 1e-6,
            infolder = "./example/data/$model/", 
            outfolder = "./example/data/$model/",   
            verbose= true, updown = updown)
    env = ADMPS.obs_env(M,Au,Ad)
...
@testset "optimize mps Ising_Triangle_good residual entropy with $atype $updown updown" for updown = [true], atype = [CuArray]
    Random.seed!(100)
    β = 100
    model = Ising_Triangle_good(1, 1, β)
